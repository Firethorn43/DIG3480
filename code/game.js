...
function Crush(pos) {
  ...
  this.wobble = Math.random() * Math.PI * 0.00001;
}
Crush.prototype.type = "crush";
...
var boostWobbleSpeed = 9, boostWobbleDist = 0.2;
var crushWobbleSpeed = 2, crushWobbleDist = 3;
...
Boost.prototype.act = function(step) {
  this.wobble += step * boostWobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * boostWobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
Crush.prototype.act = function(step) {
  this.wobble += step * crushWobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * crushWobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "crush" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
    //collects coin or boost
  } else if (type == "coin") {
